...
class IO:
    ...
    def is_exist_table(self, db_name, table_id):
        ...
        table_schema = self.sqlm.db_read("SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME=N'{}'".format(table_id))
    ...
    def create_table(self, table_nm, table_structure, db_name='qpipe', truncate_table=False):
        """
        ...
        if self.is_exist_table(db_name, table_nm):
            if truncate_table is False:
                print("Table {} already exists in DB {}. You should set 'truncate_table=True' or rename it.".format(table_nm, db_name))
                return False
            else:
            ...
            sql_ = "create table {name} ({structure})".format(name=table_nm, structure=table_struct_code)
...
class AssetIO(IO):
    def __init__(self):
        super(AssetIO, self).__init__()
        self.item_dict = dict()

    def _item_dict_initialize(self, key_nm, item_list_sql):
        df = self.sqlm.db_read(item_list_sql)
        assert len(df.columns) == 1
        self.item_dict[key_nm] = [i.lower() for i in df.ix[:, 0]]
